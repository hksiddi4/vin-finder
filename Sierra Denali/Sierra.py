...
def extractInfo(text, updated_vin):
    ...
    skip_sierra.append(updated_vin)
    with open(f"{year}/skip_sierra.txt", "a") as file:
...
def writeCSV(pdf_info):
    ...
    with open(f"{year}/{year}_sierra.csv", "a", newline='') as csvfile:
    ...
    while vinChanging <= endVIN:
        ...
        try:
            ...
            while retries < max_retries:
                try:
                    # Get Request
                    contentsGet = requests.get(newUrl, headers = {'User-Agent': 'sierra count finder', 'Accept-Language': 'en-US'}, timeout=120)
                    ...
                    # If request returns not a json content = window sticker found
                        ...
                        with open(f"{year}/sierra_{year}.txt", "a") as f:
                            f.write(f"{updated_vin}\n")
