...
def processVin(session, db_conn, urlIdent, vinChanging, endVIN, yearDig, startVIN, plant, totalIdent, urlList):
...
if __name__ == "__main__":
    ...
    default_range = 100  # Fallback range if not specified

    # Gather all potential valid trim digit series based on current model year criteria
    int_year = int(year)
    active_digits = [0, 1]  # Default/base list allocations
    if int_year >= 2023:
        active_digits.extend([3, 6])  # Z06
    if 2024 <= int_year <= 2026:
        active_digits.extend([2, 5])  # E-Ray
    if int_year >= 2025:
        active_digits.extend([4, 8])  # ZR1
    if int_year >= 2026:
        active_digits.extend([7, 9])  # ZR1X

    active_digits = sorted(list(set(active_digits)))

    # Parse max sequence limits out of your skip file grouped by series digit
    series_maxes = {}
    for seq in found_sequences:
        series_key = seq // 100000
        if series_key not in series_maxes or seq > series_maxes[series_key]:
            series_maxes[series_key] = seq

    # Build clear targets pairing the active series with their sequence max or base index fallback
    targets = []
    for digit in active_digits:
        if digit in series_maxes:
            targets.append((digit, series_maxes[digit]))
        else:
            targets.append((digit, (digit * 100000) + 1))
    ...
    with requests.Session() as session, mysql.connector.connect(**db_config) as db_conn:
        ...
        for digit, last_known_seq in targets:
            if status == "stop":
                break

            current_range = range_config.get(digit, default_range)

            # Boundaries securely constrained so ranges don't bleed into adjacent hundred-thousand blocks
            vinChanging = max((digit * 100000) + 1, last_known_seq - current_range)
            endVIN = min(((digit + 1) * 100000) - 1, last_known_seq + current_range)

            print(f"--- Processing Series Band {digit}00k ({vinChanging:06d} to {endVIN:06d}) | Search Range: {current_range} ---")
            ...
            if int_year >= 2026 and start_digit in ("7", "9"): urlChosenList = urlIdent_zr1x_list
                ...
                if start_digit not in ("0", "1"):
                    print(f"Warning: Sequence prefix '{start_digit}' unexpected. Falling back to base list.")
            ...
            for urlIdent in urlChosenList:
                ...
                status = processVin(session, db_conn, urlIdent, vinChanging, endVIN, yearDig, "1G1Y", "5", totalIdent, urlList)
                if status == "stop":
